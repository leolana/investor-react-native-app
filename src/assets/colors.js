
export const tealish = '#41D3BD'
export const tealishTwo = '#2dc2bf'
export const white = '#FFF'
export const black = '#000'
export const greyF1 = '#F1F1F1'
export const greyDD = '#DDD'
export const grey99 = '#999'
export const grey66 = '#666'
export const grey33 = '#333'
export const greyF7 = '#F7F7F7'
export const grey70 = '#707070'
export const black55 = '#555555'
export const black66 = '#666666'
export const greyEE = '#EEE'
export const greyE3 = '#E3E3E3'
export const grey88 = '#888888'
export const greyTwo = '#95989A'
export const greyF3F5 = '#F3F3F5'
export const dusk = '#434371'
export const darkDusk = '#222239'
export const greenishBlue = '#0f8b8d'
export const twilight = '#62549f'
export const rouge = '#b61843'
export const darkTwilight = '#62549F'
export const red = '#B20000'
export const redTwo = '#DD0000'
export const yellow = '#FFC400'
export const green = '#129B00'
export const greenTwo = '#40CD28'
export const blue = '#007BFF'
export const blueTwo = '#0031FF'
export const skeletonBone = '#E1E9EE'
export const skeletonHighlight = '#F2F8FC'
export const receiptColor = '#FFF8D0'
export const receiptBorderColor = '#C8C177'
export const receiptLogoColor = '#7C712A'
export const receiptTextColor = '#988A36'
export const lightBlue = '#00AEF5'
export const toastError = '#e53935'
export const toastSuccess = '#7cb342'
export const toastInfo = '#039be5'
export const lightGreen = '#00DFC1'
export const lightRed = '#FF5C5F'
export const lightYellow = '#FFD858'
export const messageBoxBackground = '#FF4E00'
export const greyAC = '#ACACAC'


// $white: #fff; // Branco
// $black: #000; // Preto
// $greenish-blue: #0f8b8d;
// $tealish: #41D3BD;
// $tealish-two: #2dc2bf;
// $dusk: #434371;
// $twilight: #62549f;
// $sand-yellow: #272727;
// $greyish-teal: #79aea5;
// $greyish-brown: #555555;
// $dark: #222222;
// $dark-dusk: #222239;
// $toxic-green: #40cd28;
// $red: #dd0000;
// $blue-light: #2DC2BF;
// $sick-green: #c1cf2e;
// $frog-green: #6fc30e;
// $medium-pink: #f04887;
// $mango: #ffb928;
// $grapefruit: #fd615d;
// $rouge: #b61843;
// $white-grey: #f2f2f2;
// $warm-grey: #999999;
// $grey: #DDDDDD;
// $grey-E3: #E3E3E3;
// $grey-DD: #DDDDDD;
// $green: #40CD28;
// $grey-EE: #EEEEEE;
// $grey-F7: #F7F7F7;
// $grey-66: #666666;
// $grey-F1: #f1f1f1;
// $grey-77: #777777;
// $blue: #02ACEF;


