...
import { IconCoin, IconPaper, IconBarcode, IcoInfo } from '../../assets/icons';

import { red, yellow, green, blue, grey99 } from '../../assets/colors';

import { CircleWithChild, Toolbar } from '../../components';

import { useSelector } from 'react-redux';

import { Request, UrlNotificacoesPegar } from '../../services';
...
export const PageNotifications = (props) => {
  ...
  const renderNotificationCard = (data) => (
    <Card>
      <CircleWithChild background={getBackgroundColor(data.item.Status)} child={getIconChild(data.item.Tipo)} />
  ...
  return (
    <SafeAreaView>
      <FlatList data={notifications} renderItem={renderNotificationCard} keyExtractor={(item) => item.id} />
    </SafeAreaView>
  );
};
