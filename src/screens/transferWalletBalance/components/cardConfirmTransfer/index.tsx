...
import { Title, Text, ItemTitle, ItemText, InfoText, Buttom } from './styles';

import { Animated, Easing } from 'react-native';

import { formatMoney, formatDate, formatBankAccountType } from '../../../../utils';
...
export const CardConfirmComponent = (props) => {
  ...
  const { index, onPress, data, navigation } = props;
    ...
    <Animated.View style={{ opacity: opacity }}>
      ...
      <Animated.View style={setAnimatedStyle('info')}>
        <InfoText>
          ...
          Agência: {data.bankData.Agencia} -{formatBankAccountType(data.bankData.TipoConta)}:{data.bankData.Conta}
        </InfoText>
      </Animated.View>
      ...
      <Animated.View style={setAnimatedStyle('input')}>
        <Buttom
          onPress={() => {
            navigation.navigate('TransferWalletBalanceConfirmation', { transferData: data });
          }}
        >
