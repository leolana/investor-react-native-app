  ...
  return (
    <SafeAreaView>
      <ScrollView>
        ...
        <Select title="Banco" options={optionsBank} onValueChange={(obj) => setBank(obj.value)} value={bank} />

        <TextInput title={'Agência'} onChangeText={(value) => setBankAgency(value)} keyboardType={'numeric'} />

        <TextInput title={'Conta'} onChangeText={(value) => setBankAccount(value)} keyboardType={'numeric'} />

        <RadioButton.Group onValueChange={(value) => setBankAccountType(value)} value={bankAccountType}>
          <ContainerLine>
            <RadioButton value="1" />
