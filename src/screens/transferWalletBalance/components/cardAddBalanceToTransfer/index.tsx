...
import { formatMoney, formatBankAccountType } from '../../../../utils';

export const CardAddBalanceToTransfer = (props) => {
  // Props

  const { index, onPress, data } = props;
  ...
  const createAnimation = (target, delay, duration = 250) =>
  ...
  const setAnimatedStyle = (target) => ({
    transform: [{ translateX: translate[target].interpolate({ inputRange: [0, 1], outputRange: [-500, 0] }) }],
  });
  ...
  return (
    <Animated.View style={{ opacity: opacity }}>
      ...
      <Animated.View style={setAnimatedStyle('input')}>
        ...
        />
        <Buttom disabled={!isValid} onPress={() => nextStep()}>
      ...
      <Animated.View style={setAnimatedStyle('info')}>
        <InfoTitle>
          Valor mínimo para transferência: <InfoText>R$ 100,00</InfoText>
        </InfoTitle>
        <InfoTitle>
          Saldo disponível para transferência: <InfoText>{formatMoney(data.walletBalance)}</InfoText>
        </InfoTitle>
        <InfoTitle>
          Custo de TED: <InfoText>R$ 0,00</InfoText>
        </InfoTitle>
      </Animated.View>
