...
import { grey99, grey66 } from '../../../../assets/colors';

import { IconInvestor } from '../../../../assets/icons';
...
export const InnitialStepComponent = (props) => {
  ...
  const generateRangeValues = () => {
    const { InvestimentoMaximo, InvestimentoMinimo, Valor, ValorCaptado, ChamadaListaEspera } = data;
    ...
    if (ChamadaListaEspera) return values.reverse();
  ...
  }, [value, reinvestimentValue, props]);
    ...
    <>
      <Retangle>
        <Text color={grey99}>
          Cidade/UF:{' '}
          <Text bold={true}>
            {data.Empresa.Endereco.Cidade} / {data.Empresa.Endereco.Uf}
          </Text>
        </Text>
      </Retangle>
      ...
      <Retangle>
        ...
        <Text bold={true}>
          ...
          <Text>{data.QuantidadeReservasInvestimento > 1 ? ' investidores' : ' investidor'}</Text>
        </Text>
      </Retangle>
      ...
      <ProgressArea>
        <ProgressHeader>
          ...
          <ProgressTitle>
            Falta: <ProgressText>{getRemainingValue()}</ProgressText>
          </ProgressTitle>
        </ProgressHeader>
      ...
      <TextInfo color={grey66}>
        Início da captação:{' '}
        <TextInfo bold={true}>{formatDate(data.InicioCaptacao, 'dd ? MMM, yyyy').replace('?', 'de')}</TextInfo>
      </TextInfo>
      <TextInfo color={grey66}>
        Expira em: <TextInfo bold={true}>{formatDate(data.FimCaptacao, 'dd ? MMM, yyyy').replace('?', 'de')}</TextInfo>
      </TextInfo>
      ...
      <TextInfo marginBottom={32} textAlign="center" color={grey99}>
        ** Ao chegar em 100% do valor solicitado, a solicitação é encerrada imediatamente e o pagamento do valor
        investido deve ser realizado em até 24 horas.
      </TextInfo>
    </>
  );
};
