...
export const OpportunitiesCardComponent = (props) => {
  ...
  const getCaptationPercent = () => {
    const { ChamadaListaEspera, ValorCaptado, Valor } = data;

    if (ChamadaListaEspera) return 100;

    let value = (ValorCaptado / Valor) * 100;

    value = value < 100.0 ? value : 100.0;
    ...
    <TouchableWithoutFeedback>
      <Card background={scoreColor}>
        <Header>
          ...
          <Flex5>
            {' '}
            <Text> {formatLoanType(data.TipoEmprestimo)} </Text>{' '}
          </Flex5>
          <Text> {data.Prazo} Meses </Text>
        </Header>

        <Body>
          <CircleWithChild background={scoreColor}>
          ...
          <Container>
            ...
            <Content>
              {' '}
              valor solicitado: <Bold> {formatMoney(data.Valor)} </Bold>{' '}
            </Content>
            <Content>
              {' '}
              Término: <Bold> {diffDaysForOpportunitie(data.FimCaptacao)} </Bold>{' '}
            </Content>
            <Content>
              {' '}
              Rendimento: <Bold> {formatPercent(data.Rendimento)} </Bold>{' '}
            </Content>
          </Container>
